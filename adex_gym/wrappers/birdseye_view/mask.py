...
from adex_gym.wrappers.birdseye_view import lanes
from adex_gym.wrappers.birdseye_view.lanes import LaneSide
...
class MapMaskGenerator:
    ...
    def route_mask(self, route: List[carla.Waypoint]) -> Mask:
        canvas = self.make_empty_mask()
        if len(route) > 2:
            line = [self.location_to_pixel(loc) for loc in route]
